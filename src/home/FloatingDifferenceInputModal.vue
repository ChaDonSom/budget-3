<template>
    <Modal ref="mainRef" @close="modals.close(id)">
        <Button
            v-if="
                latestBatchDifference &&
                latestBatchDifference.amount &&
                !isNaN(latestBatchDifference.resolved * -1)
            "
        ...
        <DollarsField
            ...
            @icon-click="
                difference.modifier = difference.modifier == 1 ? -1 : 1
            "
            autofocus
            autoselect
        ...
        <div class="flex justify-between px-4 mb-5" v-if="!modifying">
            <IconButton
                :density="-3"
                @click.stop="modifying = difference.modifier == -1 ? 1 : -1"
                >remove</IconButton
            >
            <IconButton
                :density="-3"
                @click.stop="modifying = difference.modifier"
                >add</IconButton
            >
        </div>
        <div class="flex justify-between px-4" v-if="!modifying">
            ...
            <IconButton :density="-3" @click.stop="modifying = 'x'"
                >close</IconButton
            >
        </div>
        <DollarsField
...
function modify() {
    if (modifying.value) {
        ...
        if (modifying.value == 1 || modifying.value == -1) {
            ...
            trueAmount =
                Math.ceil((trueAmount / modifyAmount.value) * 100) / 100
        }
...
function useLatestBatchDifference() {
    if (latestBatchDifference.value) {
        props.difference.modifier =
            latestBatchDifference.value?.modifier == 1 ? -1 : 1
