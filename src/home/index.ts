...
export function emergencySaving(account: AccountWithBatchUpdates) {
    const batchUpdate = account.batch_updates[0]
    const currentBalance = account.amount / 100
    const paymentToCover = Math.abs(batchUpdate.pivot.amount / 100)
    const weeks = paydaysUntil(toDateTime(batchUpdate.date)) || 1
    const perWeek = (paymentToCover - currentBalance) / weeks
...
export function minimumToMakeNextPayment(account: AccountWithBatchUpdates) {
    return (
        ...
        idealPayment(account.batch_updates?.[0]) *
            paydaysUntil(toDateTime(account.batch_updates?.[0]?.date))
    )
}

export function minimumToMakeAllExistingScheduledPayments(
    account: AccountWithBatchUpdates
) {
    const batchUpdates = account.batch_updates.filter((update) => {
        return toDateTime(update.date) > DateTime.now() && !update.done_at
    })
    const result = batchUpdates.reduce((total, update) => {
        total +=
...
export const columnsToShow = useLocalStorage(
    "budget-home-table-columns-to-show",
    {
        name: true,
        nextDate: false,
        nextAmount: false,
