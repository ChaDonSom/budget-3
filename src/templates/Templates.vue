        ...
        <div v-if="auth.authenticated" class="text-center m-3">
            ...
            <div v-if="initiallyLoaded" class="mb-5">
                <div v-if="!sortedTemplates.length" class="m-5">
                    <p>✨ No templates ✨</p>
                    <p class="m-5">
                        (Hint: make one for payday to quickly budget your money
                        the same amount every check)
                    </p>
                </div>
                <DataTable
                    @sort="updateSort"
                ...
                >
                    ...
                    <template #body>
                        <DataTableRow
                            v-for="template of sortedTemplates"
...
const initiallyLoaded = computed(() => {
    return initiallyLoadedTemplates.value && initiallySorted.value
})
    ...
    () => {
        ...
        worker.addEventListener("message", (event) => {
            if (event.data?.type == "SORT_TEMPLATES") {
                sortedTemplates.value = JSON.parse(event.data?.templates).map(
                    (a: Template & { [key: string]: any }) => {
                        const result = a
