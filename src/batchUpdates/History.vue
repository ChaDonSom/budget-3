        ...
        <div v-if="auth.authenticated" class="text-center m-3">
            <h1 class="text-xl mt-5">
                ...
                <span v-if="filteredAccountIds.length">
                    for account{{ filteredAccountIds.length == 1 ? "" : "s" }}
                    {{ filteredAccountNames.join(", ") }}
                </span>
            </h1>
            <div v-if="initiallyLoaded">
                <Button
                    @click="
                        ...
                        includeFuture =
                            enableDateRange == false ? false : includeFuture
                    "
                >
              ...
              onClose: (selectedDates: any, dateStr: any, instance: { }) => {
                ...
                <Button
                    @click="
                        ...
                        enableDateRange = includeFuture
                            ? false
                            : enableDateRange
                    "
                >
                    <template #leading-icon>{{
                        includeFuture ? "check_box" : "check_box_outline_blank"
                    }}</template
            ...
            <div v-if="initiallyLoaded" class="mb-5">
                <div v-if="!batchUpdatesValues.length" class="m-5">
                    <p>✨ No transactions ✨</p>
                    <p class="m-5">
                        (Once you move some money, or schedule it, it will show
                        up here)
                    </p>
                </div>
                <DataTable
                    v-if="batchUpdatesValues.length"
                    style="max-height: 85vh"
                >
                    <template #header>
                        <DataTableHeaderCell> Accounts </DataTableHeaderCell>
                        ...
                        >
                            <DataTableCell style="white-space: normal">
                                <div class="flex items-center">
                                    ...
                                    <span
                                        v-if="batchUpdate.accounts.length < 5"
                                    >
                                        {{
                                            batchUpdate.accounts
                                                .map((i) => i.name)
                            ...
                            <DataTableCell>{{
                                toDateTime(batchUpdate.date).toFormat("M/dd")
                            }}</DataTableCell>
                            <DataTableCell
                                numeric
                                v-tooltip="{
                                    content: `Transaction total<br>${filteredAccountNames.join(
                                        ', '
                                    )} total`,
                                    html: true,
                                }"
                            >
                                {{
                                    dollars(
                                        batchUpdate.accounts.reduce(
                                            (a, c) => a + c.pivot.amount / 100,
                                            0
                                        )
                                    )
                                }}<br />
                                <span
                                    ...
                                    v-if="filteredAccountIds.length"
                                >
                                    {{
                                        dollars(
                                            batchUpdate.accounts
                                                .filter((a) =>
                                                    filteredAccountIds.includes(
                                                        a.id
                                                    )
                                                )
                                                .reduce(
                            ...
                            <DataTableCell>
                                {{
                                    batchUpdate.note.length > 35
                                        ? batchUpdate.note.substring(0, 35) +
                                          "..."
                                        : batchUpdate.note
                                }}
                            </DataTableCell>
                    ...
                    <template #paginator>
                        <DataTablePaginator
                            ...
                            @prev-page="
                                fetchBatchUpdatesPaginated(
                                    (batchUpdates.paginator?.current_page ??
                                        0) - 1
                                )
                            "
                            @next-page="
                            ...
                            @last-page="
                                fetchBatchUpdatesPaginated(
                                    batchUpdates.paginator?.last_page ?? 0
                                )
                            "
                        />
                    </template>
...
const enableDateRange = useLocalStorage(
    "budget-history-date-range-enabled",
    false
)
...
const dateRangeArray = useLocalStorage<Date[]>(
    "budget-history-date-range-array",
    [],
    {
        serializer: {
            read: (raw: string) =>
                JSON.parse(raw).map((i: string) => new Date(i)),
...
const batchUpdatesValues = computed(
    () => batchUpdates.ordered as BatchUpdateWithAccounts[]
)
watch(
    ...
    async () => {
        ...
        if (
            filteredAccountIds.value.length &&
            filteredAccountNames.value.length < filteredAccountIds.value.length
        ) {
...
async function fetchBatchUpdatesPaginated(page?: number) {
    ...
    return await batchUpdates.fetchData({
        ...
        date_range: enableDateRange.value
            ? JSON.stringify(
                  dateRangeArray.value.map((i) =>
                      DateTime.fromJSDate(i).toFormat("yyyy-MM-dd")
                  )
              )
            : "[]",
    })
...
const filteredAccountIds = computed(() => {
    return route.query.account_id
        ...
        : (
              JSON.parse(String(route.query.account_ids ?? "[]")) as string[]
          )?.map((i) => Number(i))
})

const filteredAccountNames = computed(() => {
    return filteredAccountIds.value
        .map((i) => accounts.data[i]?.name)
        .filter((i) => i)
})

function notFilteredForAccounts(accounts: Account[]) {
    const ids = filteredAccountIds.value
    const aIds = accounts.map((a) => a.id)
    return (
        !ids.every((i) => aIds.includes(i)) ||
        !aIds.every((i) => ids.includes(i))
    )
}
...
function newBatchUpdate() {
    router.push({
        ...
        params: { id: "new" },
        query: { account_id: String(route.query.account_id) },
    })
}
</script>
