...
export const areAnyBatchDifferences = computed(() =>
...
export const accountsTotal = computed(() =>
    useAccountsStore()
        .values.map((i) => i.amount / 100)
        .reduce((a, c) => a + c, 0)
)
export const batchTotal = computed(() =>
...
export async function saveBatch() {
    ...
    const data = (await batchForm.post()) as { accounts: [] } // Will be an AccountBatchUpdate, either done, or to be done later
    ...
    for (const account of data.accounts) useAccountsStore().receive(account)
...
export class UpdateInTable {
    ...
    get amount() {
        return new Dollars(this.b.pivot.amount / 100)
    }
    get date() {
        return (function (date) {
            const me = toDateTime(date)
