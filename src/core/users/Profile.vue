<template>
    <div class="flex flex-col items-center relative">
        <h1 class="my-6 text-2xl">
            {{ form.name ? form.name : "??" }}'s Profile
        </h1>
        <Textfield v-model="form.name" :error="form.errors.name" autoselect
            >Name</Textfield
        >
        <Textfield v-model="form.email" :error="form.errors.email" autoselect
            >Email</Textfield
        >
        ...
        <h3 class="text-xl mb-4 mt-4" v-if="form.shared_users?.length">
            Currently shared with
        </h3>
        <div v-for="user of form.shared_users" class="flex items-center gap-3">
            <p>{{ user.name }}: {{ user.email }}</p>
            <IconButton v-tooltip="'Remove user'" @click="removeUser(user.id)"
                >close</IconButton
            >
        </div>
        <h3 class="text-l mb-4 mt-4" v-if="form.users_who_shared_to_me?.length">
            Currently sharing with you
        </h3>
        <div v-for="user of form.users_who_shared_to_me">
...
const form = useForm(
    ...
    JSON.parse(JSON.stringify(auth.user)) as User
)
...
async function addUser() {
    // @ts-ignore
    const result = (await addUserForm.submit("post")) as User
    form.reset(result)
}
