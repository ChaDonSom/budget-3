<template>
  <div class="flex flex-col items-center relative">
    ...
    <div class="mb-4">
      <MdcSwitch v-model="showDismissed">Show dismissed notifications</MdcSwitch>
    </div>
    ...
    <div v-else-if="!filteredNotifications.length" class="my-8">
      <p>{{ showDismissed ? 'No dismissed notifications' : 'No new notifications' }}</p>
    </div>
    ...
    <div v-else class="w-full max-w-2xl">
      <div
        ...
        :class="notification.read_at ? 'border-gray-400' : 'border-blue-500'"
      >
        <div class="flex justify-between items-start">
          ...
          <div class="flex flex-col gap-2 ml-4">
            ...
            <IconButton
              ...
              @click="undismissNotification(notification.data.uuid)"
            >
              refresh
            </IconButton>
          </div>
...
async function dismissNotification(uuid: string) {
  try {
    ...
    if (auth.user) {
      const authIndex = auth.user.notifications.findIndex(n => n.data.uuid === uuid);
      if (authIndex !== -1) {
