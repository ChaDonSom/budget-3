        ...
        <div class="mdc-select mdc-select--outlined" ref="mainRef">
            ...
            >
                <ul
                    ...
                    role="listbox"
                    :aria-label="`${accessibilityOptionType} picker listbox`"
                >
                    <slot></slot>
...
watch(
    ...
    () => {
        if (mainRef.value) {
            const selectedOption = mainRef.value.querySelector(
                "li.mdc-deprecated-list-item--selected"
            ) as HTMLElement | null
            if (selectedOption) {
                if (selectedOption.innerText)
                    optionLabelForModelValue.value = selectedOption.innerText
            }
