...
onMounted(() => {
    ...
    if (snackbar.value) {
        ...
        snackbar.value.open()
        snackbar.value.listen("MDCSnackbar:opening", (event) =>
            emit("opening", event)
        )
        snackbar.value.listen("MDCSnackbar:opened", (event) =>
            emit("opened", event)
        )
        snackbar.value.listen("MDCSnackbar:closing", (event) =>
            emit("closing", event)
        )
        snackbar.value.listen("MDCSnackbar:closed", (event) =>
            emit("closed", event)
        )
    }
})
