<template>
    ...
    <div class="fixed top-1 left-0 right-0 flex justify-center z-10">
        <transition name="offline-toast" mode="out-in">
            <div
                class="rounded-2xl bg-gray-200 px-3 py-2 flex items-center gap-2"
                v-if="!online && !down"
            >
        ...
        <VDropdown :delay="0">
            <template #default="{ show, hide, shown }">
                <transition name="auth-buttons" mode="out-in">
                    <IconButton
                        v-if="hasInitiallyLoaded && auth.authenticated"
                        @click="shown ? hide() : show()"
                    >
                        more_vert
                    </IconButton>
                </transition>
            ...
            <template #popper="{ hide }">
                <div class="p-5" @click="hide">
                    ...
                    <RouterLink :to="{ name: 'profile' }">
                        <Button
                            :disabled="
                                !auth.authenticated || $route.name == 'profile'
                            "
                    ...
                    <RouterLink :to="{ name: 'history' }">
                        <Button
                            :disabled="
                                !auth.authenticated || $route.name == 'history'
                            "
            ...
            .listen(
                ...
                (payload: { notification: Notification }) => {
                    if (auth.user?.notifications)
                        auth.user.notifications.unshift(payload.notification)
                }
            )
            .listen(
                ...
                (payload: { notification: Notification }) => {
                    ...
                    if (auth.user?.notifications) {
                        ...
                        const index = auth.user.notifications.findIndex(
                            (n) => n.data.uuid == payload.notification.data.uuid
                        )
                        if (index != -1) {
                            auth.user.notifications[index] =
                                payload.notification
                            if (payload.notification.read_at) {
                                const temp = auth.user.notifications
                                temp.splice(index)
...
watch(
    () => route.query,
    (to) => {
        if (to.notification_uuid)
            dismissNotification(String(to.notification_uuid))
    },
    { deep: true, immediate: true }
)
// This is also called when we just hit the x button to dismiss notifications directly
async function dismissNotification(id: string) {
    const response = await axios.get(`/api/dismiss-notification/${id}`)
    const index = auth.user?.notifications.findIndex(
        (i) => i.id == response.data.id
    )
    if (auth.user && index && index != -1)
        auth.user.notifications[index] = response.data
}
</script>
