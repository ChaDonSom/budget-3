<template>
    <Modal @close="attemptClose">
        <template #title>{{
            Boolean(form.name) ? form.name : "New account"
        }}</template>
        ...
        >
            ...
            on
            {{
                toDateTime(form.batch_updates?.[0]?.date).toLocaleString({
                    weekday: "long",
                    month: "long",
                    day: "numeric",
                })
            }}
            <IconButton @click="openBatchUpdate(form.batch_updates?.[0])"
    ...
    -->
        ...
        <div class="ml-4 my-4">
            <MdcSwitch v-model="isFavorite" :id="`favorite-account-${form.id}`"
                >Favorite</MdcSwitch
            >
        </div>
        <Button v-if="accountId" @click="goToAccountHistory"
            ><template #leading-icon>history</template>History</Button
        >
        <Button
            v-if="accountId && auth.user?.beta_opt_in"
            @click="goToAccountPaymentPlanning"
        >
        ...
        <transition name="error-message">
            <p
                ...
                class="bg-red-200 rounded-3xl py-3 px-4 mb-2 break-word max-w-fit text-red-800"
            >
                {{ form.errors.message }}
            </p>
        </transition>
        <template #actions>
            <div class="flex justify-between">
                <DeleteButton
                    v-if="form.id"
                    @click="deleteAccount"
                    :loading="form.processingDelete"
                    :disabled="form.processing"
                />
                <SaveButton
                    class="ml-auto"
                    @click="save"
                    :loading="form.processingNotDelete"
                    :disabled="form.processing"
                />
            </div>
        </template>
...
const form = useForm<Omit<Account, "id"> & { id: number | null }>(
    "/api/accounts",
    {
        id: null,
        name: "",
...
const isFavorite = computed({
    get: () =>
        Boolean(form.favorited_users?.find((i) => i.id == auth.user?.id)),
    set: (v) => {
        if (v) {
            if (
                auth.user &&
                !form.favorited_users?.find((i) => i.id == auth.user?.id)
            ) {
                form.favorited_users?.push(auth.user)
            }
        } else {
            form.favorited_users = form.favorited_users?.filter(
                (i) => i.id != auth.user?.id
            )
        }
    },
        ...
        try {
            await modals.confirm(
                `Do you really want to delete ${
                    accounts.data[props.accountId].name
                }?`
            )
        } catch (e) {
            return
