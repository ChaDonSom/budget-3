<template>
    <div>
        <div v-if="account" class="flex flex-col my-5">
            <h1 class="mx-2">
                Saving planning for {{ account.name }} ({{
                    DateTime.now().toFormat("M/dd")
                }}: {{ dollars(account.amount / 100) }})<br />
            ...
            <DataTable class="my-5">
                <template #header>
                    ...
                    <DataTableHeaderCell numeric
                        >Ideal weeks</DataTableHeaderCell
                    >
                    <DataTableHeaderCell numeric
                        >Ideal rate ($/w)</DataTableHeaderCell
                    >
                    <DataTableHeaderCell numeric
                        >Weeks left</DataTableHeaderCell
                    >
...
const account = computed(
    () => data.value[accountId.value] as AccountWithBatchUpdates
)
onMounted(() => {
    if (!account.value) fetchAccount(accountId.value)
})

const { minTotal, rateForWeek, updatesSorted, currentRate, ratesEachWeek } =
    usePlanning(account)
</script>
